...
### 左手側 OLED のステータス表示
USB が繋がっていない側（スレーブ）の OLED は、oledkit の既定ではロゴを出す。そこを
`oledkit_render_logo_user` の上書きで、点灯状態の表示に置き換えている。書式は
マスタ側に合わせて「4セルのラベル＋区切りグリフ `\xB1`＋値」。

```
Mode: gradient       3
HSV : 128 255 206
Zone: 3x5 block
Lock: CAP NUM SCR        <- 点灯中のものは反転表示
```

表示している値はすべてマスタから送られてくる:

| 項目 | 経路 |
|---|---|
| モード / 色相・彩度・明度 | `RGBLIGHT_SPLIT`（`RGBLED_SPLIT` により暗黙に有効） |
| LED ゾーン | 自前の `USER_SYNC_BACK` RPC |
| ロック状態 | **`SPLIT_LED_STATE_ENABLE`**（ホストは USB 側にしか通知しないため） |

モード名は乗っ取ったスロットの実際の描画内容で付けている（`flow` / `bump` /
`mirror` / `gradient`）。**この名前の対応は `RGBLIGHT_EFFECT_*` を全て有効に
したままであることが前提**。無効にすると定数自体が消え、番号も繰り上がる。

`oled_init_user` を上書きしてスレーブの180度回転を外している。あの回転は
ロゴの余白を整えるためのもので、テキストを出すなら左右で向きを揃えたほうがよい。
**もし左手側の文字が上下逆さに見えたら**、`oled_init_user` でスレーブのときだけ
`OLED_ROTATION_180` を返すように戻せばよい。

桁数の注意: 1行は 21 セルで、**ちょうど 21 文字書くとカーソルが次の行へ折り返す**。
その状態で `oled_advance_page` を呼ぶと次の行を消して飛ばしてしまうので、
各行は 21 未満に収めること（モード名を11セルに切り詰めているのはこのため）。
